...
{
   int bateau,i,j,k,z,za,clic,x,y,color;
   SDL_Event event;
   SDL_Surface * temp;
   SDL_Rect position;
	    ...
	    switch(event.type)
	    {
	      ...
	      case SDL_MOUSEBUTTONDOWN:
		           switch(event.button.button)
			   {
				...
				trans1D_2D (z,&x,&y);
			        color = placement_valide (x,y,var->flotte_hum[bateau].sens,var->flotte_hum[bateau].taille,var);
			        if(color == 1) /*placement valide*/
			           temp = var->frag_bat;
			        else
			           temp = var->touche;
			        /*on redessine le fond avec les bateaux deja pose*/
			        SDL_BlitSurface(var->background,NULL,var->ecran,&(var->position_back));
			        message_box(var,"positionement des bateaux");
       ...
       switch (var->mer_home_cpu[coord])
       {
         ...
         case 4:  var->mer_home_cpu[coord]=6;
		  ...
		  {  
                     reponse = 2;
		     FSOUND_PlaySound(FSOUND_FREE,var->coule_sound);
		     message_box(var,"Votre tir a coule un de vos bateau");
		  }
		  else
		  {
		     FSOUND_PlaySound(FSOUND_FREE,var->touche_sound);
                     message_box(var,"Votre tir a touche un de vos bateau");
		  }
		  break;
       }
   }

/**
 * \file jeu2p.c
 * \brief version 1.01 de battle_navy
...
int main(int argc, char ** argv)
{
    ...
    if(ecran == NULL)
       exit(EXIT_FAILURE);
    SDL_WM_SetCaption("Prozet zz1: Battle Navy",NULL);
    #ifdef __WIN32__
    ...
    position_menu.y = 0;
    #ifdef __WIN32__
       musique = FSOUND_Stream_Open(".\\son\\jourdefoot.mp3",FSOUND_LOOP_NORMAL,0,0);
    #else
       musique = FSOUND_Stream_Open("./son/jourdefoot.mp3",FSOUND_LOOP_NORMAL,0,0);
    #endif
    if(musique == NULL)
       exit(EXIT_FAILURE);
    FSOUND_Stream_SetLoopCount(musique,-1); /*repetition de la musique indefiniment*/
    while(finp == 0)
    {
       if(first == 1)
       {
...
int mode_local_serveur(SDL_Surface * fenetre)
{
  /*declaration des variables*/
   ...
   pthread_create(&thread_affic, NULL, (void *)thread_evenement,var);
   ...
   while(var->remain_bat_cpu != 0 && var->remain_bat_hum != 0)
   {
      ...
      if(deb == 1)
      {
         ...
         pthread_create(&thread_affic, NULL, (void *)thread_evenement,var);
	 message_box(var,"attente reponse adverse");
	 pthread_join(thread,&temp);
	 pthread_cancel(thread_affic);
      }
      ...
      pthread_create(&thread_affic, NULL, (void *)thread_evenement,var);
      message_box(var,"attente du tir adverse");
      pthread_join(thread,&temp);  
      pthread_cancel(thread_affic);
...
int mode_local_client(SDL_Surface * fenetre)
{
   ...
   SDL_BlitSurface(var->background,NULL,var->ecran,&(var->position_back));
   SDL_Flip(var->ecran);
   message_box(var,"connexion effectue");
   ...
   pthread_create(&thread_affic, NULL, (void *)thread_evenement,var);
   message_box(var,"placement des bateaux adverses");
   pthread_join(thread,&temp);  
   pthread_cancel(thread_affic);

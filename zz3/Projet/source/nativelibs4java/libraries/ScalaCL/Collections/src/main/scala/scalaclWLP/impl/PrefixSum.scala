...
private[scalaclWLP] object PrefixSum {
  ...
  private[scalaclWLP] def prefixSumByte(bitmap: CLGuardedBuffer[Boolean], output: CLGuardedBuffer[Int])(implicit context: Context) = {
  ...
  private[scalaclWLP] def prefixSumInt(bitmap: CLGuardedBuffer[Int], output: CLGuardedBuffer[Int])(implicit context: Context) = {
    CLEventBound.syncBlock(Array(bitmap), Array(output), evts => {
  ...
  """)
  private[scalaclWLP] def copyPrefixed[T](size: Int, presencePrefix: CLGuardedBuffer[Int], in: CLGuardedBuffer[T], out: CLGuardedBuffer[T])(implicit t: ClassManifest[T], context: Context) = {
